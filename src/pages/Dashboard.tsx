// --- START OF FILE src/pages/Dashboard.tsx (النسخة النهائية مع تفعيل Modal) ---
...
import HolidaysModal from '../components/HolidaysModal.tsx'; // <-- تم الاستيراد
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      const { data: holidaysData, error: holidaysError } = await supabase
        ...
        .select('*')
        .order('date', { ascending: true }); // ترتيب العطلات بالتاريخ
    ...
    <div className="relative" dir="rtl">
      ...
      <div className="z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          <div className="bg-white p-6 rounded-xl shadow-lg flex items-center justify-between"><div><p className="text-sm font-medium text-gray-500">إجمالي الموظفين</p><p className="text-3xl font-bold text-gray-900">{stats.totalEmployees}</p></div><div className="bg-blue-100 p-3 rounded-full"><Users className="text-blue-600" size={28} /></div></div>
